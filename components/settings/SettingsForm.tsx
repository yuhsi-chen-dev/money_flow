...
import { useSettingsSave } from '@/hooks/useSettings'
...
export function SettingsForm({ initialSettings }: Props) {
  ...
  const [monthlyIncome, setMonthlyIncome] = useState(
    initialSettings ? String(initialSettings.monthlyIncome) : ''
  )
  const [etfAmount, setEtfAmount] = useState(
    initialSettings ? String(initialSettings.etfAmount) : ''
  )
  const [expenses, setExpenses] = useState<FixedExpenseItem[]>(
    initialSettings?.fixedExpenses ?? []
  )
  const [defaultItems, setDefaultItems] = useState<VariableItem[]>(
    initialSettings?.defaultVariableItems ?? []
  )
